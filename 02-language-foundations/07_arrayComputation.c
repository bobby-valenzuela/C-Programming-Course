...
int main(void){
    ...
    // read num of ingredients
    scanf("%d",&numIng);
    
    // Build pricePerLb array    
    double pricePerLb[numIng];

    for(i=0;i<numIng; i++){
        scanf("%lf",&pricePerLb[i]);
    }
    
    // Build ingWeight array    
    double ingWeight[numIng];

    for(i=0;i<numIng; i++){
        scanf("%lf",&ingWeight[i]);
    }
    
    // Calculate total cost
    for(i=0;i<numIng; i++){
        // Weight by Weight per pound
        totalCost+=ingWeight[i]*pricePerLb[i];
    }
