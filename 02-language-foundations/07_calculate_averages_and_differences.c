...
The ouput shows that we are modifying all the boxcars so that they each carry a weight of 22.0 (which makes a total of 110.0 for the entire train). 
...
int main(void){
    ...
    int numCars,i;
    double currentWeight,sumWeight=0.0,avgCarWeight;
    
    // Get Num cars
    scanf("%d",&numCars);
    
    // Build car weights array
    int carWeights[numCars];
    
    // Get sum car weight
    for(i=0; i<numCars; i++){
        scanf("%lf",&currentWeight);
        sumWeight += currentWeight;
        carWeights[i]=currentWeight;
    }
    
    // Calculate diff in car weight from average
    avgCarWeight = sumWeight/numCars;
    
    for(i=0; i<numCars; i++){
        sumWeight += currentWeight;
        printf("%.1lf\n",avgCarWeight-carWeights[i]);
    }
